...
public class Berechnung {       //Funktion für den höchsten Tageswert
    ...
    public int[][] hoechsterWochenWert(int[][] daten){
        int[][][] hoechstWert = new int[3][1][1];
        hoechstWert[0][0][0] = daten[0][0];
        hoechstWert[]
    ...
    public int nidrigsterWochenWert(int[][] daten){
        int wochenMinimumWert = daten[0][0]
        for(int i = 0; i < daten.length; i++){
            int vergleichsWert = nidrigsterTagesWert(i, daten);
            if(vergleichsWert < wochenMinimumWert){
                wochenMinimumWert = vergleichsWert;
            }
        }
        return wochenMinimumWert;
    }
}
